...
#
# A third role exists only for blue/green: ECS assumes it to rewrite the listener rule during a
...
resource "aws_iam_role" "blue_green" {
  ...
  assume_role_policy = data.aws_iam_policy_document.assume_ecs[0].json
}
